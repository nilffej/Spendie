...
def get_top_tracks(term, token):
    ...
    with tempspotify.token_as(token):
        data = []
        toptracks = tempspotify.current_user_top_tracks(time_range=term,limit=50)
...
def get_top_artists(term, token):
    tempspotify = tk.Spotify()
    with tempspotify.token_as(token):
        data = []
        topartists = tempspotify.current_user_top_artists(time_range=term,limit=50)
...
def main():
    try:
        ...
        with spotify.token_as(users[user]):
            ...
            monthtracks = get_top_tracks('short_term', users[user])
            yeartracks = get_top_tracks('medium_term', users[user])
            alltimetracks = get_top_tracks('long_term', users[user])

            monthartists = get_top_artists('short_term', users[user])
            yearartists = get_top_artists('medium_term', users[user])
            alltimeartists = get_top_artists('long_term', users[user])
